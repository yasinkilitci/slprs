...
Mat srcOriginal,srcModified,srcGray,srcCropped;
...
void showDFTImage(Mat&);
void cleanImage(Mat&,int);
void calculateFilterValues(Mat&);
void drawSkewAngle(Mat&);
int readWithTesseract(Mat&,int&);
/* Function prototypes */
...
void CannyThreshold(int, void*)
{
  ...
  while(numberoftries>0)
  {
	  ...
	  /* TESSERACT */
		int confidence = 0;
	  int plate_str_len = readWithTesseract(srcCropped,confidence);
...
void cleanImage(Mat& src, int border)
{
	rectangle( src,
           Point( 0, 0 ),
...
void calculateFilterValues(Mat& src)
{
...
int readWithTesseract(Mat& srcPlate,int&confidence)
{
